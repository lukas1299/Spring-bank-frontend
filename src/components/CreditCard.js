...
const CreditCard = () => {
    ...
    const loadCreditCards = async (accountNumber) => {
        const cards = await creditCardService.getCreditCardsBelongingToAccount(accountNumber);
        setCards(cards);
    }
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            loadCreditCards(accNumber.accountId);
        };
    ...
    return (<div>
        ...
        <div style={{ display:"flex", margin:"15px", justifyContent: "flex-end" }}>
            <Fade in={checked}>
                <Alert variant="filled" severity="success" style={{width: "30%"}}>
                    Karta została zablokowana.
                </Alert>
            </Fade>
        </div>

        <div style={{ marginLeft: "5%", marginTop: "45px", display: "flex" }}>
            ...
            <div style={{ width: "70%" }}>
                <div style={{ height: "130px", marginBottom: "10px", display: "flex" }}>
                    ...
                    <div style={{ backgroundColor: "#212529", height: "100%", width: "50%", borderTopRightRadius: "10px", borderBottomRightRadius: '10px', display: "flex", alignItems: "center", justifyContent: "right" }}>
                        <div style={{ display: "flex", marginRight: "10px" }}>
                            <a style={{ color: "white", cursor: "pointer", fontWeight: "bold", marginRight: "100px" }}>Zamów nową kartę</a>
                ...
                <div style={{ backgroundColor: "#212529", height: "410px", borderRadius: "10px", display: "flex" }}>
                    {cards.map(card => (
                        <Card checked={checked} onCheckedChange={setChecked} key={card.id} cardId={card.id} cardNumber={card.cardNumber} username={card.account.username} surname={card.account.surname} expitedTime={"10/30"} status={card.cardStatus} />
                    ))}
                </div>
            </div>
        </div>
